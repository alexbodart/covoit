...
function verifUser($login,$password)
{
	...
	$admin=SQLGetChamp("SELECT admin from users where pseudo='$login'");
	$nom=SQLGetChamp("SELECT nom from users where pseudo='$login'");
	$prenom=SQLGetChamp("SELECT prenom from users where pseudo='$login'");
	if (!$id) return false; 
