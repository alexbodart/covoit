	...
	if ($action = valider("action"))
	{
		...
		*/
		...
		switch($action)
		{
			...
			break;
			case 'tripAdd' :
					if ($date = valider("dates"))
						if ($heure = valider("heure"))
							if ($place = valider("place"))
								if ($type = valider("type"))
									if ($sens = valider("sens"))
									{

											$conducteur=$_SESSION['id'];
											AjouterTrajet($date,$heure,$place,$type,$sens,$conducteur);
											$addArgs ="?view=tripAdd&succes=1";
									}
									else
										$addArgs = "?view=tripAdd&error=1";
								else
									$addArgs = "?view=tripAdd&error=2";
							else
								$addArgs = "?view=tripAdd&error=3";
						else
							$addArgs = "?view=tripAdd&error=4";
					else
						$addArgs = "?view=tripAdd&error=5";
			break;
